...
func TestNewChatTitleRoute_Endpoint(t *testing.T) {
	mockController := gomock.NewController(t)
	defer mockController.Finish()

	mockPartyReactionUseCase := use_case_mocks.NewMockPartyReactionUseCase(mockController)
	route := NewNewChatTitleRoute(mockPartyReactionUseCase)

	assert.NotEmpty(t, route.Endpoint())
	assert.Equal(t, telebot.OnNewGroupTitle, route.Endpoint())
}

func TestNewChatTitleRoute_Handle(t *testing.T) {
	mockController := gomock.NewController(t)
	defer mockController.Finish()
