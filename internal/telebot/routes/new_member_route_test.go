...
func TestNewMemberRoute_Endpoint(t *testing.T) {
	mockController := gomock.NewController(t)
	defer mockController.Finish()

	mockPartyReactionUseCase := use_case_mocks.NewMockPartyReactionUseCase(mockController)
	route := NewNewMemberRoute(mockPartyReactionUseCase)

	assert.NotEmpty(t, route.Endpoint())
	assert.Equal(t, telebot.OnUserJoined, route.Endpoint())
}

func TestNewMemberRoute_Handle(t *testing.T) {
	mockController := gomock.NewController(t)
	defer mockController.Finish()
