...
func TestVideoNoteRoute_Endpoint(t *testing.T) {
	mockController := gomock.NewController(t)
	defer mockController.Finish()

	mockPoopReactionUseCase := use_case_mocks.NewMockPoopReactionUseCase(mockController)
	route := NewVideoNoteRoute(mockPoopReactionUseCase)

	assert.NotEmpty(t, route.Endpoint())
	assert.Equal(t, telebot.OnVideoNote, route.Endpoint())
}

func TestVideoNoteRoute_Handle(t *testing.T) {
	mockController := gomock.NewController(t)
	defer mockController.Finish()
