...
func TestBaseEyesReactionUseCase_Execute(t *testing.T) {
	mockController := gomock.NewController(t)
	defer mockController.Finish()

	mockTelebotMethodService := services_mocks.NewMockTelebotMethodService(mockController)

	useCase := NewBaseEyesReactionUseCase(mockTelebotMethodService)

	chatId := int64(12345)
	messageId := 67890

	mockTelebotMethodService.EXPECT().
		SetMessageReaction(chatId, messageId, json.Eyes).
		Return(nil)
...
func TestBaseEyesReactionUseCase_Execute_Error(t *testing.T) {
	mockController := gomock.NewController(t)
	defer mockController.Finish()
