...
func TestBasePoopReactionUseCase_Execute(t *testing.T) {
	mockController := gomock.NewController(t)
	defer mockController.Finish()

	mockTelebotMethodService := services_mocks.NewMockTelebotMethodService(mockController)
	useCase := NewBasePoopReactionUseCase(mockTelebotMethodService)

	chatId := int64(12345)
	messageId := 67890

	mockTelebotMethodService.EXPECT().
		SetMessageReaction(chatId, messageId, json.Poop).
		Return(nil)
...
func TestBasePoopReactionUseCase_Execute_Error(t *testing.T) {
	mockController := gomock.NewController(t)
	defer mockController.Finish()
